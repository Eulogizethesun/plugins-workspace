...
- Added the `openLocationSettings()` JavaScript API, which jumps to the system location settings page so the user can enable the location master switch and grant permissions. OpenHarmony only; other platforms reject with a command not found or ACL error.
- The OpenHarmony ArkTS project (`openharmony/`) is now wired into the plugin build via `.ohos_path` (previously discovered only through CLI path conventions).
