...
impl<R: Runtime> AppHandleExt for tauri::AppHandle<R> {
    fn save_window_state(&self, flags: StateFlags) -> Result<()> {
        ohos_ops::save_window_state(self, flags)
    }
...
impl<R: Runtime> WindowExt for Window<R> {
    fn restore_state(&self, flags: StateFlags) -> tauri::Result<()> {
        ...
        // Platform-dispatched (ohos_ops): the OHOS variant follows the same
        // lock discipline as the OHOS save path — the cache lock is never
        // held across a dispatcher round-trip or disk I/O; the non-OHOS
        // variant keeps the original desktop code path byte-for-byte.
        ohos_ops::restore_state(self, &plugin_state, label, flags)
    }
}
...
impl<R: Runtime> WindowExtInternal for Window<R> {
    fn update_state(&self, state: &mut WindowState, flags: StateFlags) -> tauri::Result<()> {
        // The maximized/minimized queries are platform-dispatched (see
        // ohos_ops): OHOS skips these synchronous NAPI calls; desktop keeps
        // the original queries.
        let is_maximized = ohos_ops::update_state_is_maximized(self, flags)?;
        let is_minimized = ohos_ops::update_state_is_minimized(self, flags)?;
    ...
    pub fn build<R: Runtime>(self) -> TauriPlugin<R> {
        ...
        PluginBuilder::new("window-state")
                ...
                window.on_window_event(move |e| match e {
                    ...
                    WindowEvent::Moved(position) if state_flags.contains(StateFlags::POSITION) => {
                        if window_clone
                            ...
                            .is_ok()
                        {
                            // OHOS skips the is_minimized() guard — see
                            // ohos_ops::moved_is_minimized.
                            let minimized = ohos_ops::moved_is_minimized(&window_clone);
                            if !minimized {
                                let mut c = cache.lock().unwrap();
                                if let Some(state) = c.get_mut(&label) {
                    ...
                    WindowEvent::Resized(size) if state_flags.contains(StateFlags::SIZE) => {
                        if window_clone
                            ...
                            .is_ok()
                        {
                            // OHOS saves unconditionally — see
                            // ohos_ops::resized_should_save.
                            let save = ohos_ops::resized_should_save(&window_clone);
                            if save {
                                let mut c = cache.lock().unwrap();
                                if let Some(state) = c.get_mut(&label) {
                                    state.width = size.width;
            ...
            .on_event(move |app, event| {
                // Platform-dispatched (see ohos_ops): OHOS restores on Ready
                // and skips auto-save on Exit; desktop auto-saves on Exit.
                ohos_ops::on_event(app, event, state_flags)
            })
            .build()
    }
}
